...
for(i in 1:length(all_files)) {
  ...
  tmp <- read.csv(tmpfile) %>%
    ...
    dplyr::select(location, sim_type, period, date, variable, data, mean_value)
...
ggplot(all_data, aes(x = date, y = data, color = location)) +
  geom_col(width = 0.3) +
...
omega_summaries %>%
  ...
  ylab(expression(omega)) +
  xlab("") +
  coord_flip(ylim = c(0, 0.00000125)) +
  theme_minimal() +
...
```{r latents}
state_parameters %>%
  filter(parameter == "E1_0") %>%
  spread(parameter, value) %>%
  rename("L0" = E1_0) -> latents
...
for(do_state in unique(state_summaries$location)) {
  ...
  dat <- tmp_summary %>%
    filter(!is.na(data)) %>%
    dplyr::select(date, variable, data) %>%
