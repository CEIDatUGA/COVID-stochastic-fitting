...
author: 
  ...
  - Éric Marty^[Odum School of Ecology and Center for the Ecology of Infectious Diseases, University of Georgia]
  - Eamon B. O'Dea^[Odum School of Ecology and Center for the Ecology of Infectious Diseases, University of Georgia]
...
Note that transmission rate at time $t$ is $\omega(t) = \frac{\beta}{N} \psi(t) \phi(t) \Gamma(t)$, resulting the force of infection $f(t) = \omega(t) \left(I(t) \right)$, where, for simplicity, $I$ stands for all infectious individuals at time $t$.
...
We then performed two rounds of MIF, each for 150 iterations with 2,000 particles and geometric cooling. For some states, we increase the number of iterations in the first round of MIF to 350. 
...
# pops <- readRDS(here::here("data/us_popsize.rds")) %>%
...
omega_summaries <- state_summaries %>%
  ...
  filter(sim_type == "status_quo", date < Sys.Date()) %>% 
  pivot_wider(names_from = variable, values_from = mean_value)
  
omega_summaries %>% ggplot(aes(date, omega)) +
...
omega_summaries %>% 
  ...
  summarise(mean_omega = mean(omega),
            lower = quantile(omega, 0.1),
            upper = quantile(omega, 0.9)) %>%
  ...
  coord_flip() +
  theme_minimal() +
  ggtitle("Mean transmission (omega) over time", subtitle = "error bars show min and max")
```
...
#ggplot(latents, aes(y = value, x = reorder(location, L1_0))) +
ggplot(latents, aes(y = MLE, x = reorder(location,MLE))) +
...
# ggplot(latents, aes(x = L1_0, y = omega)) +
#   geom_point() +
...
all_maes <- tibble()
all_mases_lag7 <- tibble()
all_mases_lag1 <- tibble()
logliks <- tibble()
...
for(do_state in unique(state_summaries$location)) {
  ...
  # observed data with 7-day moving average
  dat_ma <- dat %>%
  ...
  mae_total <- tmp_summary %>%
    ...
    left_join(dat_ma, by = c("date", "variable")) %>%
    drop_na() %>%
  ...
  # MASEs
  mase_total_lag7 <- tmp_summary %>%
    filter(variable %in% c("daily_cases", 
                           "daily_deaths")) %>%
    filter(sim_type %in% c(NA,"status_quo")) %>%
    filter(period == "Past") %>%
    mutate(variable = as.character(variable)) %>%
    dplyr::select(date, variable, mean_value) %>%
    rename("value" = mean_value) %>%
    left_join(dat_obs, by = c("date", "variable")) %>% # raw observations
    group_by(variable) %>%
    yardstick::mase(truth=observation,estimate=value,m=7L) %>% # assumes weekly seasonality
    ungroup() %>%
    select(variable, mase = .estimate) %>% 
    mutate(location = do_state)
  
  all_mases_lag7 <- bind_rows(all_mases_lag7, mase_total_lag7)
  
  mase_total_lag1 <- tmp_summary %>%
    filter(variable %in% c("daily_cases", 
                           "daily_deaths")) %>%
    filter(sim_type %in% c(NA,"status_quo")) %>%
    filter(period == "Past") %>%
    ...
    left_join(dat_obs, by = c("date", "variable")) %>% # raw observations
    group_by(variable) %>%
    yardstick::mase(truth=observation,estimate=value,m=1L) %>% # assumes no seasonality
    ungroup() %>%
    select(variable, mase = .estimate) %>% 
    mutate(location = do_state)

  all_mases_lag1 <- bind_rows(all_mases_lag1, mase_total_lag1)
...
# save MAEs as file
all_maes %>% saveRDS(paste0(here::here("output/"),"us_all_maes.rds"))
# save MASEs as file
all_mases_lag1 %>% saveRDS(paste0(here::here("output/"),"us_all_mases_nonseasonal_naive.rds"))
all_mases_lag7 %>% saveRDS(paste0(here::here("output/"),"us_all_mases_seasonal_naive.rds"))
...
ggplot(all_mases_lag7 %>%
         ...
         mutate(variable = ifelse(variable == "daily_cases", 
                                  "Daily reported cases", 
                                  "Daily reported deaths")), 
       aes(x = reorder(location, mase), y = mase)) +
  geom_col(aes(fill = variable)) +
  facet_wrap(~variable, scales = "free_x") +
  geom_hline(yintercept=1,linetype = "dashed",color = "black") +
  annotate(geom = "text", x = 0, y = 1, label = "MASE = 1", color = "black", angle = 90, hjust = 0, vjust=1.5) +
  theme_minimal() +
  coord_flip() +
  scale_fill_manual(values = c("dodgerblue", "salmon")) +
  guides(fill = FALSE) +
  xlab("") +
  ylab("Mean absolute scaled error (MASE)") 
```
