...
for(do_state in unique(state_summaries$location)) {
  ...
  mase_total <- tmp_summary %>%
    ...
    dplyr::select(date, variable, mean_value) %>%
    rename("value" = mean_value) %>%
    left_join(dat_obs, by = c("date", "variable")) %>% # use with mase(m=7L)
    # left_join(dat_ma, by = c("date", "variable")) %>% # use with mase(m=1L)
    group_by(variable) %>%
    yardstick::mase(truth=observation,estimate=value,m=7L) %>% # use with raw observed data
    # yardstick::mase(truth=observation,estimate=value,m=1L) %>% # use with ma of data
    ungroup() %>%
    select(variable, mase = .estimate) %>% 
    mutate(location = do_state)

  all_mases <- bind_rows(all_mases, mase_total)
...
Here we show the MASEs for each state for daily case reports and daily death reports.
