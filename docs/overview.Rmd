...
current_path <- here::here("output/current/")

simfiles <- list.files(path = here::here('output/current'), ".csv")
paramfiles <- list.files(path = here::here('output/current'), "params-natural.rds")
paramfiles_transformed <- list.files(path = here::here('output/current'), "parameter-estimates")
state_metadata <- readRDS(here::here('data/us_popsize.rds'))
...
all_maes <- readRDS(paste0(here::here("output/"),"us_all_maes.rds"))
...
dailycases.threshold <- 30 # new cases per day per 100k
...
notgoodfits <- state_metadata[!state_metadata$state_full %in% goodfits,]$state_full
ntopstates <- 9
```
...
States ommited due to poor model fits to data: `r notgoodfits`.
...
#                         & dailycases$median_value_per100k > dailycases.threshold,] %>% 
#   aggregate(median_value_per100k ~ location, data = ., max) %>% 
#   arrange(desc(median_value_per100k)) %>%
#   pull(location)

# top states (with good fits) by max daily cases / 100 k in status quo scenario
topstates <- dailycases[dailycases$date > foredate 
                        & dailycases$sim_type == 'status_quo'
                        & dailycases$location %in% goodfits,] %>% 
  aggregate(median_value_per100k ~ location, data = ., max) %>% 
  arrange(desc(median_value_per100k)) %>%
  pull(location)
topstates <- topstates[1:ntopstates]

# plot
dailycases_subset <- dailycases %>%
  filter(location %in% topstates) %>% 
  mutate(location = factor(location, levels = topstates))
...
# Landing Page figure
# select a few states
selectstates <- c('Georgia', topstates[1:2])
dailycases_subset <- dailycases %>%
