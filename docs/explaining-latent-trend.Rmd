...
tmp <- state_summaries %>%
  ...
  filter(variable %in% c("actual_daily_cases", "daily_cases")) %>%
  filter(period == "Past") %>%
...
dat <- tmp %>%
  filter(variable == "actual_daily_cases") %>%
  filter(mean_value < 20000)

ggplot(dat, aes(x = date, y = mean_value)) +
  geom_point() +
  geom_line(data=fit, color = "orange", size = 1) +
...
all_trends %>%
  ...
  geom_line(size = 1) +
  theme_minimal(base_size = 14) +
  labs(x = NULL, y = expression(R[e])) -> p2
...
all_trends %>%
  ...
  geom_line(size = 1) +
  theme_minimal(base_size = 14) +
  scale_color_brewer(type = "qual", name = NULL, labels = c("latent", "mobility")) +
  labs(x = "Date", y = "Relative effect\non tranmission") +
  theme(legend.position = "bottom") -> p3

cowplot::plot_grid(p1, p2, p3, ncol = 1, rel_heights = c(1, 1, 1.2), align = "v")
ggsave("ga_example.png", height = 6, width = 10, units = "in", dpi = 360)
```
...
# figure with cumulative mandates and relative mobility trend
all_mandates_mobility <- trends_with_mandates %>% 
  filter(variable %in% c("num_mandates", "mobility_trend")) %>%
  pivot_wider(names_from = variable, values_from = value) %>%
  group_by(date) %>%
  summarise(total_mandates = sum(num_mandates),
            mean_mobility = mean(mobility_trend),
            .groups = "drop") %>%
  mutate(relative_mandates = total_mandates / max(total_mandates, na.rm = TRUE)) %>%
  dplyr::select(-total_mandates) %>%
  pivot_longer(cols = mean_mobility:relative_mandates)

# max mobility after april
changepoint <- all_mandates_mobility %>%
  filter(date > "2020-04-30") %>%
  filter(name == "mean_mobility") %>%
  filter(value == max(value))

ggplot(all_mandates_mobility, aes(x = date, y = value)) +
  geom_line(aes(color = name), size = 2) +
  geom_vline(aes(xintercept = as.Date("2020-04-24")), linetype = 2, color = "grey35") +
  annotate(geom = "text",
  ...
  labs(x = "Date", y = "Relative value") +
  theme(legend.position = "top")
ggsave("relative_mobility_mandates.png", width = 8, height = 4, units = "in", dpi = 360)
```


## Key dates for retrospective analysis
...
key_dates <- tibble(
  date = c("2020-03-13", 
           ...
           "2020-11-26",
           "2020-12-25"),
  event = c("US President declares nationwide emergency.",
            "White House extende social distancing measures.",
            "CDC announces mask wearing guidelines outside of the home.",
...
median_trends <- trends_with_mandates %>%
  filter(variable != "num_mandates") %>%
  group_by(date, variable) %>%
  summarise(median_value = median(value))

median_diffs <- median_trends %>%
  pivot_wider(names_from = variable, values_from = median_value) %>%
  mutate(difference = latent_trend - mobility_trend) %>%
  left_join(key_dates)

ggplot(median_diffs, aes(x = date, y = difference)) +
  geom_hline(aes(yintercept = 0)) +
  geom_point(aes(color = mobility_trend), size = 3) +
  geom_text(aes(label = event), angle = 90) +
  scale_color_viridis_c()

# trends_with_mandates %>%
#   filter(variable != "num_mandates") %>%
#   ggplot(aes(x = date, y = value)) +
#   geom_line(alpha = 0.1, aes(group = location)) +
#   geom_line(data = median_trends, aes(y = median_value), color = "blue", size = 2) +
#   geom_text(data = key_dates, aes(x = date, y = 1, label = event), angle = 90) +
...
ggplot(output, aes(x = Label, y = PropVar)) +
  ...
  labs(x = NULL, y = NULL) +
  scale_fill_brewer(type = "qual", name = NULL, labels = c("latent", "mobility"))  +
  scale_y_continuous(breaks = c(0, 1)) +
  ggthemes::theme_few() +
  theme(legend.position = "top")
ggsave("anova_re_states.png", width = 10, height = 7, units = "in", dpi = 360)
```
...
cowplot::plot_grid(p3, p4, p1, p2, ncol = 2)
ggsave("cross-corrs.png", height = 8, width = 10, units = "in", dpi = 360)
