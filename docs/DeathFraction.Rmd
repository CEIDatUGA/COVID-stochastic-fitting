...
plot(m1,type='l', ylim=c(0,1),main=paste0("n={", paste(n,collapse=","),"}"))
...
n <- 1:4
t <- 1:100
t_half <- log_half_dead <- 50
m_max <- max_frac_dead <- .8
m_min <- min_frac_dead <- .1

# dmax - (dmax-dmin)*(t/(t+halfdead))
m_exting <- max_frac_dead - (max_frac_dead - min_frac_dead) * (t/(t+log_half_dead))
m_new <- m_max-(m_max-m_min)*(t^n[1]/(t^n[1] + t_half^n[1]))
