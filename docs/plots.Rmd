...
for(i in 1:length(allfiles)) {
  ...
  firstcasedate <- stateresults$date %>% min()
  ...
  state_extended_results <- stateresults %>%
    ...
    mutate(prevalence = daily_all_infections / (statepop-cumulative_deaths)) %>% 
    # calculate omega
    rename(omega = combined_trend) %>% 
    # calculate mean S
    mutate(S = (statepop - cumulative_all_infections) / (statepop - cumulative_deaths)) %>% 
    # calculate q
    ...
    # calculate mean R_e
    mutate(R_e = getReff(S = S, N = 1, # whole population
                         omega = omega,
                         q = q,
                         s = s,
                         params = statepars_allmle)) %>%
  ...
  #   ggplot(aes(x = date, y = median_value/statepop)) +
  #   geom_ribbon(aes(ymin = lower_80/statepop, ymax = upper_80/statepop), alpha = 0.2) +
  #   geom_line() +
  #   # geom_line(data = dat, aes(x = date, y = mean_value), color = "blue") +
  #   ylab("S/N") +
  #   ggtitle(statename) -> g_S
  ...
  # Reproduction number
  state_extended_results %>%
    filter(variable %in% c("R_e")) %>%
    filter(sim_type == "status_quo") %>%
    ggplot(aes(x = date, y = mean_value)) +
    geom_line() +
    # geom_point(aes(color = variable)) +
    ggtitle("") +
    xlab("") + ylab("R_e") +
    theme(legend.position = "none") -> g_re
  ...
  figure <- ggarrange(g_cases, g_deaths, g_trends, g_re, g_deathfrac,
