  ...
  for(i in 1:ncol(x)){
    for(j in 1:ncol(x)){
      ...
      g <- ggplot2::autoplot(ccf) + theme_minimal() +
        ylim(-1,1) + 
        labs(title=paste0(names(all_re_wide)[i]," ", names(all_re_wide)[j]), x = "", y="")
      mlist[i,j] <- list(ccf)
      glist[j,i] <- list(g)
    }
  }
...
re_ccf_matrix <- ccfmatrix(all_re_wide[,1:4])
...
grobs <- re_ccf_matrix$plots
gridExtra::grid.arrange(grobs=grobs, ncol=4, nrow = 4)
