...
for(i in 1:length(all_files)) {
  ...
  tmp <- read.csv(tmpfile) %>%
    ...
    mutate(prevalence = daily_all_infections / state_pop) %>% 
...
burnin <- c(state_summaries %>% pull(date) %>% min(), as.Date("2020-02-25"))
...
mean_phi <- all_phi %>% group_by(date) %>% summarise(phi = mean(mean_value))
...
mean_psi <- all_psi %>% group_by(date) %>% summarise(psi = mean(mean_value))
...
mean_omega <- all_omega %>% group_by(date) %>% summarise(omega = mean(mean_value))
...
CA %>% filter(date < future[1]) %>% 
  filter(date > burnin[2]) %>%
  ggscatterhist(x = "mobility_trend", y = "omega", 
