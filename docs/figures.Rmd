...
first plot: x = omega; y = daily prevalence (all infections / pop size)
```{r}
...
state_summaries_wide %>% 
  ...
  ggplot(aes(x = omega, y = prevalence)) +
  geom_line(size = 0.5) +
  # ylab("Latent trend\n(% of normal)") +
  # scale_y_continuous(limits = c(0,1), labels = scales::percent) +
...
plot(x = lag(CA$omega,0), y = CA$prevalence)
plot(x = lag(CA$omega,7), y = CA$prevalence)
plot(x = lag(CA$omega,14), y = CA$prevalence)

plotly::plot_ly(data = CA, type = 'scatter', mode = 'markers',
                x = lag(~omega,0), y = ~prevalence, 
                hoverinfo = 'text', 
                text = ~paste('</br> Omega: ', omega,
