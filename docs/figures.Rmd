...
ccfmatrix <- function(x){
  ...
  for(i in 1:ncol(x)){
    for(j in 1:ncol(x)){
      ccf <- ccf(x = x[,i], y= x[,j], na.action = na.pass, ylab = "CCF", plot = FALSE)
      g <- ggplot2::autoplot(ccf)+ylim(-1,1)+labs(title=paste0(i," ", j), x = "", y="")
      mlist[i,j] <- list(ccf)
      glist[j,i] <- list(g)
    }
  }
...
re_ccf_matrix <- ccfmatrix(all_re_wide)
