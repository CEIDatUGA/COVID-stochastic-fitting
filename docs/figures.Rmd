...
weighted_mean_phi <- all_phi %>% group_by(date) %>% summarise(phi = sum(phi*relative_pop))

weighted_mean_phi[weighted_mean_phi$date < "2020-03-13",]$phi <- NA
...
g_phi <- ggplot(all_phi, aes(x = date, y = phi, color = location)) +
  ...
  geom_line(data = weighted_mean_phi, alpha = .6, size = 2, color = 'red') +
...
p_phi$x$data[[51]]$text <- gsub('red','Weighted mean of all states', p_phi$x$data[[51]]$text)
...
weighted_mean_psi <- all_psi %>% group_by(date) %>% summarise(psi = sum(psi*relative_pop))

weighted_mean_psi[weighted_mean_psi$date < "2020-03-13",]$psi <- NA
...
g_psi <- ggplot(all_psi, aes(x = date, y = psi, color = location)) +
  ...
  scale_color_viridis_d(option = "D", direction = -1, alpha = .6, end = .75) +
  # theme_dark(base_line_size = 0.5) +
  geom_line(data = weighted_mean_psi, alpha = .6, size = 2, color = 'red') +
  annotate("rect", xmin = future[1], xmax = future[2], ymin = 0, ymax = 1, alpha = .2) +
  theme_minimal() +
  guides(color = FALSE)
...
p_psi$x$data[[51]]$text <- gsub('red','Weighted mean of all states', p_psi$x$data[[51]]$text)
...
weighted_mean_omega <- all_omega %>% group_by(date) %>% summarise(omega = sum(omega*relative_pop))

weighted_mean_omega[weighted_mean_omega$date < "2020-03-13",]$omega <- NA
...
g_omega <- ggplot(all_omega, aes(x = date, y = omega, color = location)) +
  ...
  scale_color_viridis_d(option = "D", direction = -1, alpha = .6, end = .75) +
  # theme_dark(base_line_size = 0.5) +
  geom_line(data = weighted_mean_omega, alpha = .6, size = 2, color = 'red') +
...
p_omega$x$data[[51]]$text <- gsub('red','Weighted mean of all states', p_omega$x$data[[51]]$text)
...
weighted_mean_re[weighted_mean_re$date < "2020-03-13",]$R_e <- NA
...
plotly::orca(p_re, "R_e.svg")

```

```{r}
p_trends <- plotly::subplot(p_phi, p_psi, p_omega, p_re, nrows = 4, shareX = TRUE, titleY = TRUE) %>% 
  plotly::layout(showlegend = FALSE)

plotly::orca(p_trends, "trends.svg")
...
c) predictor = AUC of omega; response = cumulative all infections [WRONG]
...
# orca(p_corr, "corr_cumI_AUomega.svg")
```
c.2) predictor = AUC of omega; response = cumulative all infections (fraction)

```{r corr_allinfections_auomega}
# state_summaries_wide %>% 
#   filter(period == 'Past') %>%
#   filter(date > burnin[2]) %>%
#   group_by(location) %>% 
...
corrdf %>%
  mutate(cumulative_all_infections_fraction = 1 - susceptible_fraction) %>% 
  summarise(AU_omega = max(AU_omega), cumulative_all_infections_fraction = max(cumulative_all_infections_fraction)) %>% 
  ggstatsplot::ggscatterstats(x = "AU_omega", y = "cumulative_all_infections_fraction", type = "spearman", 
                              title = "Correlation of cumulative infections with Area under omega")
...
```{r}
library(forecast) # needed for ggplot2 to be able to plot acf objects
all_re_wide <- all_re %>% 
...
# default heatmap of coor matrix
...
make_heatmap(corrmatrix.upper.melt)
make_heatmap(corrmatrix.reordered.upper.melt)
