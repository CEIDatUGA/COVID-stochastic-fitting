...
for(i in 1:length(all_files)) {
  ...
  tmp <- read.csv(tmpfile) %>%
    ...
    dplyr::select(location, sim_type, period, date, variable, median_value)
...
plot_data <- function(database, state, legend = FALSE) {
  ...
  statedata <- database %>% filter(location == state) %>% select(date, variable, median_value) %>% 
  ...
  p_nowcast <- plotly::plot_ly(data = statedata, x = ~date, showlegend = legend) %>% 
...
We measure social distancing using aggregated, anonymized locations recorded by location-based mobile phone apps, colected by <a href="https://www.unacast.com/covid19/social-distancing-scoreboard" target="_blank">Unacast</a>.
...
all_phi <- state_summaries %>%
  ...
  dplyr::select(location, date, median_value) %>%
  filter(date <= Sys.Date())

g <- ggplot(all_phi, aes(x = date, y = median_value, color = location)) +
  geom_line(size = 0.5) +
  # ylab("Human movement\n(% of normal)") +
