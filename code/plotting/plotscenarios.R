...
filename_label <- mif_res$filename_label
pomp_model <- mif_res$pomp_model

out_sims <- scenario_res$sims
covar_scens <- scenario_res$covars
...
dates_df <- mif_res$pomp_data %>% select(date, time) %>% rename(Date = date)
...
### plotly part not working, leaving it as ggplot for now
plotly_lp <- lp
