runmif_allstates <- function(parallel_info, mif_settings, pomp_list, par_var_list)
{
  ...
  if (parallel_run == FALSE)
  {
    ...
    {
      all_pfs = list() #lists containing results for each MIF for a given state
      out_mif = list()
      pomp_model = pomp_list[[n]]$pomp_model    
  ...
  if (parallel_run == TRUE)
  {
    ...
    all_mifs <-  foreach(n=1:length(pomp_list), .combine='cbind') %:%
                {
                  foreach(i=1:mif_settings$replicates) %dopar% 
                  {
                      run_mif(pomp_model = pomp_list[[n]]$pomp_model , 
                              num_mif_iterations = mif_num_iterations, 
                              params = c(param_start[i,],fixed_params), 
                              num_particles = mif_num_particles, 
                              c_frac = mif_cooling_fracs, 
                  ...
                  } #end inner loop over starting values
                } #end outer loop over states
    stopCluster(cl)
