/* pomp C snippet file: pomp_abeb56942878a9b735803ca0ec959e16 */
/* Time: 2020-04-14 15:03:44.228 -0400 */
/* Salt: D6FA4EA54FC31F9E144FD9D9 */
...
{
 lik = dnorm(log_beta_s, -17.0927194398423, 8, 1) + dnorm(trans_e, 2, 8, 1) + dnorm(trans_a, 0, 8, 1) + dnorm(trans_c, 1, 8, 1) + dnorm(trans_h, 10, 8, 1) + dnorm(log_g_e, 0, 8, 1) + dnorm(log_g_a, 0.133531392624523, 8, 1) + dnorm(log_g_su, -0.405465108108164, 8, 1) + dnorm(log_g_sd, 0.287682072451781, 8, 1) + dnorm(log_g_c, 0.287682072451781, 8, 1) + dnorm(log_g_h, -1.09861228866811, 8, 1) + dnorm(log_max_diag, 0, 8, 1) + dnorm(log_diag_inc_rate, -3, 8, 1) + dnorm(max_detect_par, 0, 8, 1) + dnorm(log_detect_inc_rate, -3, 8, 1) + dnorm(frac_asym, 1.5, 8, 1) + dnorm(frac_hosp, 3, 8, 1) + dnorm(frac_dead, 1.2, 8, 1) + dnorm(log_theta_cases, 2.30258509299405, 8, 1) + dnorm(log_theta_hosps, 2.30258509299405, 8, 1) + dnorm(log_theta_deaths, 2.30258509299405, 8, 1) + dnorm(E1_0, 40, 8, 1) + dnorm(E2_0, 40, 8, 1) + dnorm(E3_0, 40, 8, 1) + dnorm(E4_0, 40, 8, 1) + dnorm(Ia1_0, 22, 8, 1) + dnorm(Ia2_0, 22, 8, 1) + dnorm(Ia3_0, 22, 8, 1) + dnorm(Ia4_0, 22, 8, 1) + dnorm(Isu1_0, 90, 8, 1) + dnorm(Isu2_0, 90, 8, 1) + dnorm(Isu3_0, 90, 8, 1) + dnorm(Isu4_0, 90, 8, 1) + dnorm(Isd1_0, 14, 8, 1) + dnorm(Isd2_0, 14, 8, 1) + dnorm(Isd3_0, 14, 8, 1) + dnorm(Isd4_0, 14, 8, 1) ; 
...
void R_init_pomp_abeb56942878a9b735803ca0ec959e16 (DllInfo *info)
{
R_RegisterCCallable("pomp_abeb56942878a9b735803ca0ec959e16", "__pomp_load_stack_incr", (DL_FUNC) __pomp_load_stack_incr);
R_RegisterCCallable("pomp_abeb56942878a9b735803ca0ec959e16", "__pomp_load_stack_decr", (DL_FUNC) __pomp_load_stack_decr);
R_RegisterCCallable("pomp_abeb56942878a9b735803ca0ec959e16", "__pomp_dprior", (DL_FUNC) __pomp_dprior);
}
